...
const TEAM_COLORS = {
  SRH: "#F7A721", GT: "#0B2135", PBKS: "#ED1B24", RCB: "#2B2A29",
  RR: "#EA1A85", CSK: "#FFFF00", KKR: "#3A225D", DC: "#00008B",
  MI: "#004BA0", LSG: "#0057E2",
};

// CRICKET-SMART OVER CONVERTER: Handles 19.3 as 19.5 and fixes typos like 19.8
const toDecimal = (ov) => {
...
const initialData = {
  standings: [
    ...
    { team: "RCB", p: 10, w: 6, l: 4, pts: 12, scored: 1950, faced: 198.0, conceded: 1720, bowled: 200.0 },
    { team: "RR", p: 11, w: 6, l: 5, pts: 12, scored: 2040, faced: 216.3, conceded: 2020, bowled: 220.0 },
    { team: "CSK", p: 10, w: 5, l: 5, pts: 10, scored: 1780, faced: 198.0, conceded: 1750, bowled: 200.0 },
    ...
    { team: "MI", p: 10, w: 3, l: 7, pts: 6, scored: 1720, faced: 200.0, conceded: 1850, bowled: 200.0 },
    { team: "LSG", p: 10, w: 3, l: 7, pts: 6, scored: 1680, faced: 200.0, conceded: 1860, bowled: 200.0 },
  ],
  matches: [
    { id: 53, home: "CSK", away: "LSG", hR: "", hO: "20", aR: "", aO: "20", win: "", status: "upcoming" },
    { id: 54, home: "RCB", away: "MI", hR: "", hO: "20", aR: "", aO: "20", win: "", status: "upcoming" },
    ...
    { id: 66, home: "GT", away: "CSK", hR: "", hO: "20", aR: "", aO: "20", win: "", status: "upcoming" },
    { id: 67, home: "SRH", away: "RCB", hR: "", hO: "20", aR: "", aO: "20", win: "", status: "upcoming" },
    { id: 68, home: "LSG", away: "PBKS", hR: "", hO: "20", aR: "", aO: "20", win: "", status: "upcoming" },
    { id: 69, home: "MI", away: "RR", hR: "", hO: "20", aR: "", aO: "20", win: "", status: "upcoming" },
    { id: 70, home: "KKR", away: "DC", hR: "", hO: "20", aR: "", aO: "20", win: "", status: "upcoming" },
  ],
};
  ...
  const calculateTable = () => {
    ...
    matches.filter(m => m.status === "upcoming").forEach((m) => {
      ...
      if (!isNaN(hr) && !isNaN(ar)) {
        ...
        h.faced += toDecimal(m.hO); h.bowled += toDecimal(m.aO);
        a.faced += toDecimal(m.aO); a.bowled += toDecimal(m.hO);
      }
    });
    return table.map((t) => {
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#1e3a8a", color: "white" }}>
              ...
              <th>P</th><th>PTS</th><th>NRR</th>
            </tr>
          </thead>
          <tbody>
            {standings.map((t, i) => (
              <tr key={t.team} style={{ borderBottom: "1px solid #e2e8f0", background: i < 4 ? "#f0fdf4" : "white" }}>
                <td style={{ padding: "12px", textAlign: "center" }}>{i + 1}</td>
                <td style={{ fontWeight: "bold", borderLeft: `6px solid ${TEAM_COLORS[t.team]}` }}>&nbsp;{t.team}</td>
                <td style={{ textAlign: "center" }}>{t.p}</td>
                <td style={{ textAlign: "center" }}><strong>{t.pts}</strong></td>
                <td style={{ textAlign: "center", color: parseFloat(t.nrrDisplay) >= 0 ? "#15803d" : "#b91c1c" }}>{t.nrrDisplay}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        {matches.filter(m => m.status === "upcoming").map((m) => (
          <div key={m.id} style={{ background: "white", padding: "18px", borderRadius: "12px", border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", gap: "15px" }}>
              <div style={{ flex: 1 }}>
                <button onClick={() => updateMatch(m.id, "win", m.home)} style={{ width: "100%", padding: "10px", borderRadius: "6px", border: "1px solid #cbd5e1", background: m.win === m.home ? TEAM_COLORS[m.home] : "white", color: m.win === m.home ? "white" : "black", fontWeight: "bold", cursor: "pointer" }}>{m.home}</button>
                <div style={{ display: "flex", gap: "4px", marginTop: "8px" }}>
                  <input style={{ width: "50%", padding: "6px", borderRadius: "4px", border: "1px solid #cbd5e1" }} type="number" placeholder="Runs" value={m.hR} onChange={(e) => updateMatch(m.id, "hR", e.target.value)} />
                  <input style={{ width: "50%", padding: "6px", borderRadius: "4px", border: "1px solid #cbd5e1" }} type="text" placeholder="Overs" value={m.hO} onChange={(e) => updateMatch(m.id, "hO", e.target.value)} />
                </div>
              </div>
              <div style={{ alignSelf: "center", fontWeight: "bold", color: "#94a3b8" }}>VS</div>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: "4px", marginTop: "8px" }}>
                  <input style={{ width: "50%", padding: "6px", borderRadius: "4px", border: "1px solid #cbd5e1" }} type="text" placeholder="Overs" value={m.aO} onChange={(e) => updateMatch(m.id, "aO", e.target.value)} />
                  <input style={{ width: "50%", padding: "6px", borderRadius: "4px", border: "1px solid #cbd5e1" }} type="number" placeholder="Runs" value={m.aR} onChange={(e) => updateMatch(m.id, "aR", e.target.value)} />
                </div>
              </div>
            </div>
          </div>
